...
public class KataRunner {
    ...
    private List<KataObserver> kataObservers;
    ...
    public void addKataObserver(KataObserver ko) {
        if (kataObservers == null)
            kataObservers = new ArrayList<>();
        kataObservers.add(ko);
    }
...
//			System.out.println("\nExited with error code : " + exitCode);
    ...
    private int testAnalyze(KataBrain brain, String sgfPath, String fileName, VisDetector vis, int onlySearchTurn, int maxVisits) throws Exception {
        ...
        for (int resultsProcessed = 0; resultsProcessed < query.analyzeTurns.size(); resultsProcessed++) {
            ...
            for (KataObserver kataObserver : kataObservers) {
                kataObserver.updateLeftBoard(n);
            }
            ...
            if (kprev != null) {
                ...
                if (detector.validProblem) {
                    ...
                    if (vis.newDetection(brain, detector, fileName)) {
                        foundCount++;
                        for (KataObserver kataObserver : kataObservers) {
                            kataObserver.updateRightBoard(n);
                            kataObserver.updateFoundCount(foundCount);
                            kataObserver.updateSolutions(detector.numSols);
                            kataObserver.updateStoneDelta(detector.totDelta);
                        }
